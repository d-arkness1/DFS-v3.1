...
namespace VoxelBusters.ScreenRecorderKit
{
    public abstract class GifRecorder : MonoBehaviourZ, IScreenRecorder
    {
        ...
        public static void ShareRecording(string text = null, string subject = null,
            CompletionCallback callback = null)
        {
            callback?.Invoke(false, ScreenRecorderError.FeatureUnsupported(ErrorDomain));
        }

        public static void OpenRecording(GifTexture source, CompletionCallback callback = null)
        {
            // Gaurd case
            if (source == null)
            {
                callback?.Invoke(false, new Error(ErrorDomain, ScreenRecorderErrorCode.kUnknown, "Source is null."));
                return;
            }
        ...
        string IScreenRecorder.Name => Name;

        public ScreenRecorderState State => m_state;
        ...
        void IScreenRecorder.OpenRecording(CompletionCallback callback)
        {
            OpenRecording(LastPreview, callback);
        }
        ...
        protected virtual void OnStateChange(ScreenRecorderState state)
        {
            if (state == ScreenRecorderState.Record)
            {
